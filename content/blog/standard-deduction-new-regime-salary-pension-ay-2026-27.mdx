---
{
  "title": "Standard Deduction in New Regime for Salary and Pension in AY 2026-27",
  "description": "Check standard deduction treatment for salary and pension cases in AY 2026-27 while comparing old and new regime, Form 16, and final taxable income.",
  "slug": "standard-deduction-new-regime-salary-pension-ay-2026-27",
  "publishedAt": "2026-05-27T00:00:00.000Z",
  "modifiedAt": "2026-05-27T00:00:00.000Z",
  "primaryKeyword": "standard deduction new regime AY 2026-27",
  "secondaryKeywords": [
    "salary",
    "pension",
    "new regime",
    "Form 16"
  ],
  "contentType": "explainer",
  "faqs": [],
  "id": "standard-deduction-new-regime-salary-pension-ay-2026-27",
  "excerpt": "Check standard deduction treatment for salary and pension cases in AY 2026-27 while comparing old and new regime, Form 16, and final taxable income.",
  "categoryId": "tax-regime",
  "coverImage": "/assets/blog/text-covers/standard-deduction-new-regime-salary-pension-ay-2026-27.svg",
  "authorId": "mye-ca-editorial",
  "authorName": "MyeCA Editorial Team",
  "authorRole": "ITR Filing Desk",
  "authorBio": "The MyeCA Editorial Team prepares evidence-led guides for Indian taxpayers, freelancers, founders, and small businesses.",
  "seoTitle": "Standard Deduction in New Regime for Salary and Pension in AY 2026-27 | MyeCA",
  "seoDescription": "Check standard deduction treatment for salary and pension cases in AY 2026-27 while comparing old and new regime, Form 16, and final taxable income.",
  "keyHighlights": [
    "Use taxable income, not CTC.",
    "Check Form 16 computation.",
    "Regime comparison should include final salary records."
  ],
  "relatedPostIds": [
    "salary-tax-guide-fy-2025-26-ay-2026-27",
    "zero-tax-12-lakh-section-87a-rebate-ay-2026-27",
    "multiple-form-16-job-change-itr-ay-2026-27",
    "form-16-part-a-part-b-reconciliation-ay-2026-27"
  ],
  "ctaLabel": "Estimate Tax",
  "ctaHref": "/calculators/income-tax",
  "isFeatured": false,
  "readingTimeMinutes": 6,
  "createdAt": "2026-05-27T00:00:00.000Z",
  "tags": [
    "standard deduction new regime AY 2026-27",
    "salary",
    "pension",
    "new regime",
    "Form 16"
  ],
  "audience": "individuals",
  "targetAudience": "Indian taxpayers or business owners resolving standard deduction new regime AY 2026-27 from current rules and source records.",
  "sourceLinks": [
    {
      "label": "Income Tax Department - Salaried Individuals AY 2026-27",
      "url": "https://www.incometax.gov.in/iec/foportal/help/individual/return-applicable-1",
      "checkedAt": "2026-05-27"
    },
    {
      "label": "Income Tax Department - Tax Credit Mismatch FAQs",
      "url": "https://www.incometax.gov.in/iec/foportal/node/11487",
      "checkedAt": "2026-05-27"
    }
  ],
  "serviceSlug": null,
  "calculatorSlug": "income-tax",
  "canonicalUrl": null,
  "userIntent": "informational",
  "keyTopics": [
    "Use taxable income, not CTC.",
    "Check Form 16 computation.",
    "Regime comparison should include final salary records."
  ],
  "qualityStatus": "needs_revision"
}
---

# Standard Deduction in New Regime for Salary and Pension in AY 2026-27

Check standard deduction treatment for salary and pension cases in AY 2026-27 while comparing old and new regime, Form 16, and final taxable income.

Standard deduction sounds simple. For many salaried individuals and pensioners it is — but the details still catch people out. The most common error is running a regime comparison using gross salary or CTC rather than taxable income. The second most common is assuming that because the employer applied the standard deduction in Form 16, the return will carry it correctly regardless of regime choice. Neither assumption is safe without verifying the return computation.

This is a pre-filing review note for FY 2025-26 returns in AY 2026-27. It is not a promise of a specific refund or a guarantee of notice avoidance. The goal is to help you approach the return with the right documents and the right questions.

## What matters most for standard deduction 2026-27

| Point | What it means |
| --- | --- |
| 1 | Use taxable income, not CTC. |
| 2 | Check Form 16 computation. |
| 3 | Regime comparison should include final salary records. |

## Rule to confirm for standard deduction 2026-27

The Income Tax Department's guidance for salaried individuals in AY 2026-27 covers return eligibility, income head treatment, and regime options. Standard deduction is available under the new regime as well as the old — but the deduction amount, the regime rules, and the interaction with other income heads need to be verified in the return preview, not assumed from a payslip or a news article.

| Reference | Link |
| --- | --- |
| income Tax Department - Salaried Individuals AY 2026-27 | [Open source](https://www.incometax.gov.in/iec/foportal/help/individual/return-applicable-1) |
| Income Tax Department - Tax Credit Mismatch FAQs | [Open source](https://www.incometax.gov.in/iec/foportal/node/11487) |

## How standard deduction 2026-27 works in practice

Consider a pensioner who also has fixed deposit interest. The pension slip shows the gross pension, Form 16 (if issued) reflects the deductions applied by the payer, and AIS shows both the pension and the bank interest reported separately. Before treating the prefilled return as final, the pensioner should cross-check the pension figure, confirm the standard deduction appears correctly under the chosen regime, and verify that the bank interest from AIS matches what has been declared under the other sources head.

Review your return in three passes. First, check the assessment year and your taxpayer profile — resident individual, pensioner, senior citizen, or otherwise. Second, confirm which income heads apply and whether the ITR form selected is correct. Third, match tax credits, TDS deducted by the bank or pension payer, and the standard deduction treatment against your Form 16, pension certificate, AIS, and Form 26AS. A mismatch left unresolved is how refund delays and demand notices begin.

## Standard deduction: Documents to reconcile for standard deduction 2026-27

- Form 16 or pension certificate
- AIS and Form 26AS
- Bank interest certificate
- Regime comparison sheet
- Final return preview

Keep these together with a brief note explaining why the taxable income figure is what it is. When a Form 16 amount differs from what AIS shows — which happens — that note is what makes the position explainable.

## Standard deduction: Checks that support standard deduction 2026-27

- For standard deduction, confirm AY 2026-27 is the correct assessment year for FY 2025-26 income.
- Use taxable salary from Form 16 Part B, not CTC, as the starting point for computation.
- In the standard deduction working, compare Form 16 or Form 16A with AIS, TIS, Form 26AS, and bank records.
- Run the regime comparison on final salary records, not headline numbers.
- Note every mismatch, correction, deduction, or refund position and file only after the figures are defensible. Then e-verify within the allowed timeline.

## What can go wrong with standard deduction 2026-27

- Comparing old and new regime on gross salary or CTC rather than taxable income after permissible deductions.
- Ignoring bank interest reported in AIS, which is a common source of tax-credit mismatches.
- Treating payroll computation as final without checking Form 16 Part B against the return preview.
- Missing e-verification, which leaves the return technically not filed even after submission.

The most expensive error is reaching for the wrong resolution tool after something goes wrong. For standard deduction, a revised return, a rectification request, an AIS feedback, a demand payment, and a notice reply address different problems. Use each only when you know what exactly needs to be corrected.

## Standard deduction: Related filing paths for standard deduction 2026-27

- [Salary tax guide](/blog/salary-tax-guide-fy-2025-26-ay-2026-27)
- [Zero tax section 87A](/blog/zero-tax-12-lakh-section-87a-rebate-ay-2026-27)
- [Income tax calculator](/calculators/income-tax)
- [Choose your ITR form](/itr/form-selector)
- [Regime comparator](/calculators/regime-comparator)
- [Start CA-assisted filing](/expert-consultation)

Use these tools as a preparation step, not a substitute for checking your actual documents. Cases with capital gains, foreign assets, business income, a large refund, or a tax-credit mismatch need a closer review before filing.

For broader reading, [the complete AY 2026-27 filing guide](/blog/complete-ay-2026-27-itr-filing-guide) and [the form-selection guide](/blog/itr-form-selection-master-guide-ay-2026-27) are useful complements to this article. CA-assisted filing is available at [expert consultation](/expert-consultation).

## Standard deduction: Build the evidence file for standard deduction

Before filing, document the assessment year, taxpayer status, ITR form, income heads, tax regime choice, source records, and the reason each major figure appears in the return. Where the position is time-sensitive — Form 16 issue date, AIS update, TDS return processing, e-verification deadline, or notice response window — record the relevant date alongside the decision.

For standard deduction new regime AY 2026-27, the minimum file should include the computation, portal downloads, source statements, challans, acknowledgement, and correspondence. This article is educational and should not be applied to a specific individual's return without examining that person's own documents.

## Standard deduction: practical next action

Use taxable income, not CTC. Verify the Form 16 computation. Run any regime comparison on final salary records. Standard deduction in the new regime is available, but its correct treatment depends on the regime selected, the income head structure, and what the return computation actually shows. A clean return comes from consistent, verified treatment across all records — not from assumptions.

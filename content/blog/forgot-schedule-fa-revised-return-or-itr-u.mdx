...
If the revised return window is open, revision may be the cleaner route. ITR-U is restricted and generally not useful where the update does not increase tax liability. Foreign asset omissions should be reviewed carefully.

If foreign asset disclosure was missed, this guide explains revised return, ITR-U limits, notices, and why expert review is important.
...
Revised return and updated return have different time limits and conditions. Updated returns generally cannot be used to reduce tax or increase refund and are constrained by additional tax rules.
...
If Schedule FA was missed but there is no additional income tax liability, ITR-U may not be available as an easy correction path. Expert review is strongly recommended.
...
- Check if revised return deadline is open.
- Identify omitted foreign assets.
- Compute any omitted income.
- Review notice or email received.
- Document valuation and source.
...
- Assuming ITR-U fixes every omission.
- Ignoring foreign asset notices.
- Waiting past revised return deadline.
- Filing without valuation support.
...
Revised return is time-sensitive. ITR-U has restrictions. Schedule FA omissions are high-risk.
...
For this specific topic, the reviewer should document the working position for "I Forgot Schedule FA. Can I File Revised Return or ITR-U?" using the taxpayer's facts, the selected AY 2026-27 form, the records used for computation, and the reason each major number appears in the return. The note should explicitly mention whether the issue affects form selection, income classification, deduction eligibility, tax credit matching, refund timing, notice response, or disclosure schedule completion.

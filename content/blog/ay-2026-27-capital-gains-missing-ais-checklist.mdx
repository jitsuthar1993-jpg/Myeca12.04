---
{
  ...
  "targetAudience": "Investors and traders calculating and reporting capital gains missing AIS AY 2026-27 from broker, sale, and cost records.",
...
## AIS-capital-gains closing evidence

AIS-capital-gains work begins by naming the period, taxpayer, and event before any amount is copied into a schedule. AIS-capital-gains records should be matched to the statement, certificate, contract, ledger, portal download, or notice that actually supports the fact. AIS-capital-gains differences need a dated explanation that distinguishes timing from ownership, classification, correction, or missing evidence. AIS-capital-gains calculations should preserve inputs, formula, source version, and rounding rather than only the final total. AIS-capital-gains files should end with the figure used, the next action, and the current official instruction checked for the relevant form, rate, eligibility condition, or deadline.

For capital gains missing from AIS, keep corporate-action notices and old broker statements because the present broker report may not contain the acquisition history.

Retain the acquisition trail even when the omitted sale is small, because the same cost history may be needed for a later disposal.

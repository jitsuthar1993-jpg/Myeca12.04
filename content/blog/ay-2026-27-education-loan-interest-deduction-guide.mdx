...
## education-loan closing evidence

education-loan work begins by naming the period, taxpayer, and event before any amount is copied into a schedule. education-loan records should be matched to the statement, certificate, contract, ledger, portal download, or notice that actually supports the fact. education-loan differences need a dated explanation that distinguishes timing from ownership, classification, correction, or missing evidence. education-loan calculations should preserve inputs, formula, source version, and rounding rather than only the final total. education-loan files should end with the figure used, the next action, and the current official instruction checked for the relevant form, rate, eligibility condition, or deadline.

For education-loan interest, confirm the borrower and eligible student from the lender documents before treating a family payment as the taxpayer deduction.

Exclude principal and unsupported charges from the interest line, and keep the lender statement that shows the distinction.

Keep the sanction letter and annual certificate together when the lender later revises the interest breakup.

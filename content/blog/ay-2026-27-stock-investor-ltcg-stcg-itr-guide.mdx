...
## stock-investing closing evidence

stock-investing work begins by naming the period, taxpayer, and event before any amount is copied into a schedule. stock-investing records should be matched to the statement, certificate, contract, ledger, portal download, or notice that actually supports the fact. stock-investing differences need a dated explanation that distinguishes timing from ownership, classification, correction, or missing evidence. stock-investing calculations should preserve inputs, formula, source version, and rounding rather than only the final total. stock-investing files should end with the figure used, the next action, and the current official instruction checked for the relevant form, rate, eligibility condition, or deadline.

...
const evidenceLogPath = path.resolve("docs/bing-search-console-evidence-log.csv");
...
const requiredItems = BING_SEARCH_REQUIRED_EVIDENCE_ITEMS;

const urlInspectionExpectations = new Map(
  PRIORITY_ITR_SEARCH_ROUTES.map((route) => {
    const label =
      route === "/" ? "homepage"
      : route === "/blog" ? "blog hub"
      : route === "/blog/when-will-itr-filing-start-ay-2026-27" ? "priority blog article"
      : route === "/services/itr-for-salaried" ? "salaried service"
      : route === "/calculators/income-tax" ? "income tax calculator"
      : route === "/itr/form-selector" ? "form selector"
      : route === "/form16-parser" ? "Form 16 parser"
      : route === "/itr-season-2026" ? "ITR season hub"
      : "salary guide";
    return [`Bing URL Inspection ${label}`, toAbsoluteUrl(route)] as const;
  }),
);
...
function main() {
  ...
  if (!fs.existsSync(evidenceLogPath)) {
    checks.push({
      label: "Bing evidence log exists",
      ok: false,
  ...
  checks.push({
    label: "Bing required evidence items",
  ...
  checks.push({
    label: "Bing evidence required fields",
  ...
  const urlInspectionProblems = [...urlInspectionExpectations.entries()].filter(([item, url]) => {
    ...
    return !row?.evidence.includes(url);
  });
  checks.push({
    label: "Bing URL Inspection queue URLs",
  ...
  const readiness = evaluateSearchGoalReadiness([{ name: "Bing", csv, requiredItems }]);
  const latestPendingRows = readiness.pendingExternalEvidence;
  checks.push({
    label: "Bing pending external evidence remains explicit",
    ok: true,
    detail: `${latestPendingRows.length} latest pending_external item(s) remain before completion can be claimed`,
  });
  checks.push({
    label: "Bing latest evidence readiness policy",
  ...
  if (checks.some((check) => !check.ok)) {
    console.error("\nBing indexing evidence log check failed.");
    process.exit(1);
  }

  console.log("\nBing indexing evidence log is structurally ready.");
}

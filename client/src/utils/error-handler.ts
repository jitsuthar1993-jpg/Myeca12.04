...
class ErrorHandler {
  ...
  private createNotification(errorInfo: ErrorInfo): HTMLElement | null {
    ...
    icon.textContent = this.getIconForSeverity(errorInfo.severity);
    ...
    title.textContent = this.getTitleForSeverity(errorInfo.severity);
    ...
    close.className = 'error-notification-close';
    close.type = 'button';

...
interface CalcLayoutProps {
  ...
  variant?: "blue" | "emerald" | "indigo" | "amber" | "violet" | "orange" | "purple" | "rose" | "teal" | "cyan" | "green" | string;
  // SEO Content Props
  blogTitle?: string;
              ...
              <section className="space-y-12">
                <div className="text-center space-y-4">
                  <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-emerald-50 text-emerald-600 border border-emerald-100/50">
                    ...
                    <span className="text-xs font-normal uppercase tracking-widest">Compliance Authority</span>
                  </div>
                  <h2 className="text-3xl font-normal text-slate-900 tracking-tight">Tax & Legal Compliance Hub</h2>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  {complianceFacts.map((fact, i) => (
                    <div key={i} className="p-8 rounded-[2.5rem] bg-white border border-slate-100 shadow-sm hover:shadow-md transition-all">
                      ...
                      <h4 className="text-lg font-normal text-slate-900 mb-3">{fact.title}</h4>
                      <p className="text-slate-500 font-normal text-sm leading-relaxed">{fact.content}</p>
                    </div>
                  ))}
                </div>
              </section>
              ...
              <section className="space-y-12 pb-20">
                <div className="text-center space-y-4">
                  <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-blue-50 text-blue-600 border border-blue-100/50">
                    ...
                    <span className="text-xs font-normal uppercase tracking-widest">The How-To Vault</span>
                  </div>
                  <h2 className="text-3xl font-normal text-slate-900 tracking-tight">Frequently Asked Questions</h2>
                </div>
                <div className="max-w-3xl mx-auto space-y-4">
                  {faqs.map((faq, i) => (
                    <div key={i} className="p-8 rounded-[2rem] bg-white border border-slate-100 shadow-sm">
                      <h4 className="text-lg font-normal text-slate-900 mb-4 flex gap-3">
                        <span className="text-primary/40 font-normal">Q.</span> {faq.q}
                      </h4>
                      <div className="flex gap-3">
                        <span className="text-emerald-500/40 font-normal">A.</span>
                        <p className="text-slate-500 font-normal text-sm leading-relaxed">{faq.a}</p>
                      </div>
                    </div>
                  ))}
                </div>

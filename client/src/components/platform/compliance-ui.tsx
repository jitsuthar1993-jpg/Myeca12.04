...
export const navItems = [
  ...
  { label: "File ITR", href: "/which-itr-form-to-file?source=workspace_quick_nav", icon: FileText },
          ...
          <div key={item.label} className="relative rounded-2xl border border-slate-200 bg-slate-50 p-4">
            <div
              className={cn(
                ...
                complete ? "bg-emerald-700 text-white" : "bg-white text-brand-600 ring-1 ring-slate-200",
              )}
            >
              {index + 1}
            </div>
    ...
    <div className="min-h-screen bg-[#f6f9fd] text-slate-950">
      <header className="sticky top-0 z-40 border-b border-slate-200/80 bg-white/90 backdrop-blur-xl">
        <div className="mx-auto flex h-16 max-w-[1500px] items-center gap-4 px-4 sm:px-6 lg:px-8">
          ...
          <div className="ml-auto flex items-center gap-2">
            ...
            <UserCircle className="hidden h-9 w-9 text-brand-600 sm:block" />
          </div>
        </div>
      </header>
      ...
      <div className="mx-auto grid max-w-[1500px] lg:grid-cols-[280px_1fr]">
        <aside className="sticky top-16 hidden h-[calc(100vh-4rem)] border-r border-slate-200/80 bg-white/75 p-4 lg:block">
          ...
          <div className="mt-6 rounded-[24px] bg-brand-600 p-5 text-white">
        ...
        <main className="min-w-0 px-4 pb-24 pt-6 sm:px-6 lg:px-8">
          <div className="mb-6 flex flex-col gap-4 rounded-[32px] bg-gradient-to-br from-brand-600 via-brand-800 to-[#082a5c] p-6 text-white shadow-[0_30px_90px_-50px_rgba(0,48,135,0.75)] md:flex-row md:items-end md:justify-between">
            <div>
...
export function UploadDropzone({
  ...
  return (
    <button
      type="button"
      onClick={onClick}
      className="group flex w-full flex-col items-center justify-center rounded-[28px] border-2 border-dashed border-blue-200 bg-blue-50/60 p-8 text-center transition hover:border-brand-600 hover:bg-blue-50"
    >
      <div className="rounded-2xl bg-white p-4 text-brand-600 shadow-sm transition group-hover:-translate-y-1">
